...
export default function RandomTable() {
  ...
  const [formData, setFormData] = useState(new CreateRandomMeetingDto(
  ...
  const createRandomMeetingMutation = useCreateRandomMeeting();
  const deleteRandomMutation = useDeleteRandom();
  const cancelRandomMutation = useCancelRandom();

  // API 호출 파라미터 설정
  const apiParams = {
    page: Math.max(0, currentPage || 0), // currentPage가 유효하지 않으면 0으로 설정
    date: "",
    district: formData.randomDistrict 
  };

  // 랜덤 모임 리스트 API 호출
  const { data: meetingListResponse, isLoading, error, refetch } = useRandomMeetingList(apiParams);
  ...
  // 지역 변경 핸들러
  const handleRegionChange = (region: RandomDistrict) => {
    setFormData(prev => ({ ...prev, randomDistrict: region }));
    setCurrentPage(0); // 지역 변경 시 첫 페이지로 리셋
  };

  const handleSubmit = () => {
    const createRandomMeetingDto = new CreateRandomMeetingDto(
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedItems) {
        await deleteRandomMutation.mutateAsync(id);
      }
  ...
  const handleCancelMatching = async () => {
    ...
    try {
      for (const id of selectedItems) {
        await cancelRandomMutation.mutateAsync(id);
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <button
                type="button"
                onClick={() => handleRegionChange(RandomDistrict.GANGNAM)}
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  formData.randomDistrict === RandomDistrict.GANGNAM
                    ? "bg-purple-600 text-white"
                    : "bg-gray-200 text-gray-700 hover:bg-gray-300"
                }`}
              >
                {getRandomDistrictKorean(RandomDistrict.GANGNAM)}
              </button>
              <button
                type="button"
                onClick={() => handleRegionChange(RandomDistrict.HONGDAE_HAPJEONG)}
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  formData.randomDistrict === RandomDistrict.HONGDAE_HAPJEONG
                    ? "bg-purple-600 text-white"
                    : "bg-gray-200 text-gray-700 hover:bg-gray-300"
                }`}
              >
                {getRandomDistrictKorean(RandomDistrict.HONGDAE_HAPJEONG)}
              </button>
            </div>
          </div>
